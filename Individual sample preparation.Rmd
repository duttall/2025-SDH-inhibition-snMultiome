...
sample_list <- list()
  ...
  srt.dat<- Read10X_h5(paste(sample,"_CBfiltered_seurat.h5", sep = ""))
srt.frag <- paste(sample,"_atac_fragments.tsv.gz", sep = "")
...
srt[["peaks"]] <- CreateChromatinAssay(counts = macs_counts, fragments = Fragments(srt), annotation = annotation)
...
combined.peaks <- unlist(combined.peaks)
...
srt[['peakunion']] <- CreateChromatinAssay(counts = newcounts,fragments = Fragments(srt),annotation = annotation)
...
for (sample in sample_list){
  srt <-sample
srt <-  SCTransform(srt,vars.to.regress = "percent.mt",vst.flavor = "v2", method = "glmGamPoi",variable.features.n = 5000)
...
agg.list <- PrepSCTIntegration(object.list = sample_list, anchor.features = features)
